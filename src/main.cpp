...
//settings
char *lightName = "Hue WS6812 strip op ESP01";
...
NeoPixelBus<NeoGrbwFeature, NeoEsp8266Uart0Ws2812xMethod> *strip = NULL;
...
// lightengine wijzigt de kleuren van de leds naar de kleuren zoals in een light is opgegeven. De verandering wordt gemaakt over een bepaalde periode.
void lightEngine()
{
  for (int light = 0; light < lightsCount; light++) //voor elk licht. Dus de engine kan ook per licht gemaakt worden, maar hoe zit
  {
    if (lights[light].lightState) //als licht aan staat
    {
      ...
      {
        inTransition = true;            //in transition betekent dat de huidige kleur afwijkt van de eindkleur van één van de kleuren van de lamp.
        for (uint8_t k = 0; k < 4; k++) //voor elke led kleur (RGBW)
        {
          if (lights[light].colors[k] != lights[light].currentColors[k])  // als de eindkleur nog niet de huidige kleur is,
            lights[light].currentColors[k] += lights[light].stepLevel[k]; // dan de kleur een stapje wijzigen
          if ((lights[light].stepLevel[k] > 0.0 && lights[light].currentColors[k] > lights[light].colors[k]) || (lights[light].stepLevel[k] < 0.0 && lights[light].currentColors[k] < lights[light].colors[k]))
            lights[light].currentColors[k] = lights[light].colors[k]; // als de huidige kleur voorbij de eindkleur komt, dan is de huidige kleur de eindkleur
        }

        if (lightsCount > 1) //als er meerdere lampen zijn
        {
          if (light == 0) // eerste lamp
          {
          ...
          {
            for (int pixel = 0; pixel < dividedLightsArray[light]; pixel++) //dividedlightsarray geeft de startled aan van de volgende lamp. Dus of in het strip-object opnemen  waar elke light begint, of dat opnemen in elke light
            {
              long pixelSum;
    ...
    else
    {
      if (lights[light].currentColors[0] != 0 || lights[light].currentColors[1] != 0 || lights[light].currentColors[2] != 0)
      {
        ...
        for (uint8_t k = 0; k < 3; k++)
        {
          ...
          if (lights[light].currentColors[k] < 0)
            lights[light].currentColors[k] = 0;
        }
        if (lightsCount > 1)
        {
          if (light == 0)
...
void handleSwitches()
{
  if (hwSwitch == true)
  {
    ...
    {
      if (switchButton.onPressed())
      {
        lights[light].lightState = true;
      }
      else if (switchButton.onPressedLong())
      {
        lights[light].bri += 56;
      ...
      else if (switchButton.offPressed())
      {
        lights[light].lightState = false;
      }

      else if (switchButton.offPressedLong())
      {
        lights[light].bri -= 56;
...
void restoreState()
{
  ...
  for (JsonPair state : json.as<JsonObject>())
  {
    ...
    // fill array alights with lights

    alights[lightId].lightState(values["on"]);
    alights[lightId].bri((uint8_t)values["bri"]);
    if (values.containsKey("x"))
    {
      alights[lightId].x(values["x"]);
      alights[lightId].y(values["y"]);
      alights[lightId].colorMode(1);
    }
    else if (values.containsKey("ct"))
    {
      alights[lightId].ct(values["ct"]);
      alights[lightId].colorMode(2);
    }
    else
    {
      if (values.containsKey("hue"))
      {
        alights[lightId].hue(values["hue"]);
        alights[lightId].colorMode(3);
      }
      if (values.containsKey("sat"))
      {
        alights[lightId].sat((uint8_t)values["sat"]);
        alights[lightId].colorMode(3);
      }
    }

    // fill vector vlight with lights
    addlight = new light;

    addlight->lightState((bool)values["on"]);
    addlight->bri((uint8_t)values["bri"]);
    if (values.containsKey("x"))
    {
      addlight->x(values["x"]);
      addlight->y(values["y"]);
      addlight->colorMode(1);
    }
    else if (values.containsKey("ct"))
    {
      addlight->ct(values["ct"]);
      addlight->colorMode(2);
    }
    else
    {
      if (values.containsKey("hue"))
      {
        addlight->hue(values["hue"]);
        addlight->colorMode(3);
      }
      if (values.containsKey("sat"))
      {
        addlight->sat((uint8_t)values["sat"]);
        addlight->colorMode(3);
      }
    }
    vlight.push_back(*addlight);
  }
}
...
void ChangeNeoPixels(uint16_t newCount)
{
  ...
  strip = new NeoPixelBus<NeoGrbwFeature, NeoEsp8266Uart0Ws2812xMethod>(newCount); // and recreate with new count
  strip->Begin();
}
                ...
                for (JsonPair state : root.as<JsonObject>())
                {
                  ...
                  if (values.containsKey("alert") && values["alert"] == "select")
                  {
                    if (lights[light].lightState)
                    {
                      ...
                      lights[light].currentColors[3] = 0;
                    }
                    else
                    {
                      lights[light].currentColors[2] = 126;
                      lights[light].currentColors[3] = 126;
                    }
                  }
